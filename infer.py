...
if __name__ == "__main__":
    ...
    ## Required parameters
    parser.add_argument("--data_dir", default='./text', type=str, required=False,
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result, preds_results = evaluate(args, model, tokenizer, prefix=prefix)
            result = dict((k + '_{}'.format(global_step), v) for k, v in result.items())
            results.update(result)

        ind_counts = 0
        for item in preds_results:
            for x in item:
                print(ind_counts,Contradictory_inds[x])
                ind_counts += 1
